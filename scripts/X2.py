...
for well in R0:  # for each well that is in R0
    ...
    stat = matching(R0_ovr_seg, RX_ovr_seg, criterion='iou', thresh=iou_cutoff, report_matches=True)
    # print(stat)
    ...
    df = pd.DataFrame(list(zip([x[0] for x in stat[14]], [x[1] for x in stat[14]], stat[15])),
                      columns=['R0_label', 'RX_label', 'iou'])
    df_filt = df[df['iou'] > iou_cutoff]
    print('removed', len(df) - len(df_filt), 'out of', len(df), 'RX organoids that are not matched to R0')
    ...
    df_filt['segmentation_ovr_' + names[1]] = RX.plates[plate_id].wells[well_id].segmentations[ovr_channel]
