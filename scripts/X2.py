            ...
            for property in regionprops(ovr_seg):
                ...
                spatial_index.insert(
                    ...
                    properties={
                        ...
                        "plate_id": well.plate.plate_id,
                        "round_id": int(
                            re.search(r"\d+", rnd).group()
                        ),  # extract integer from round name
                        "channel_id": int(
                            re.search(r"\d+", ovr_channel).group()
                        ),  # extract integer from channel name
                        "label": property.label,
...
with spatial_index:
    ...
    manhattan_dist = func.abs(R0_x_centroid - RX_x_centroid) + func.abs(
        R0_y_centroid - RX_y_centroid
    )
    ...
    matching_df = pd.DataFrame(
        ...
        .filter(
            ...
            & (R0_desc.round_id < RX_desc.round_id)
            & (manhattan_dist < min_region_dist)  # prevents as R0 == RX
        )
        .all()
    )
...
matching_df["round_delta"] = matching_df.apply(
    lambda row: row["RX_round_id"] - row["R0_round_id"], axis=1
)

R0_groups = matching_df.groupby(
    ["well_id", "channel_id", "plate_id", "R0_round_id", "R0_label"]
)
...
RX_groups = matching_df.groupby(
    ["well_id", "channel_id", "plate_id", "RX_round_id", "RX_label"]
)
...
for rid in matching_df.index:
    ...
    if R0_obj_id is None:
        with spatial_index:
            next_obj_id = (
                spatial_index.session.query(
                    func.ifnull(func.max(spatial_index.Descriptor.object_id), 0) + 1
                )
                .subquery()
                .as_scalar()
            )
            ...
            spatial_index.session.query(spatial_index.Descriptor).filter(
                spatial_index.Descriptor.id == int(row["R0_desc_id"])
            ).update({"object_id": next_obj_id}, synchronize_session="fetch")
            spatial_index.session.commit()

            R0_obj_id = (
                spatial_index.session.query(spatial_index.Descriptor.object_id)
                .filter(spatial_index.Descriptor.id == int(row["R0_desc_id"]))
                .one()[0]
            )
...
matching_df_filt = matching_df.loc[
    matching_df["similarity_score"] > min_similariry_score
].copy(deep=True)
matching_df_filt
...
for well in R0:  # for each well that is in R0...
    ...
    df_save["segmentation_ovr_" + names[1]] = (
        RX.plates[p].wells[w].segmentations[ovr_channel]
    )
