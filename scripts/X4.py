...
if not len(nuc_feat_df_list) == 0:
    ...
    nuc_df = nuc_df.sort_values(
        by=["hcs_experiment", "root_dir", "plate_id", "well_id", "org_label", "nuc_id", "channel_id"])
...
if not len(mem_feat_df_list) == 0:
    ...
    mem_df = mem_df.sort_values(
        by=["hcs_experiment", "root_dir", "plate_id", "well_id", "org_label", "mem_id", "channel_id"])
...
if not len(cell_feat_df_list) == 0:
    ...
    cols_to_move = ["hcs_experiment", "root_dir", "plate_id", "well_id", "org_label", "nuc_id_linked", "channel_id",
                    "nuc_id", "mem_id"]
    cell_df = cell_df[cols_to_move + [col for col in cell_df.columns if col not in cols_to_move]]
    cell_df = cell_df.sort_values(
        by=["hcs_experiment", "root_dir", "plate_id", "well_id", "org_label", "nuc_id_linked", "channel_id"])
...
for i in range(len(round_names) - 1):
    ...
    right["id"] = right["plate_id"] + "_" + right["well_id"] + "_" + right["organoid_id"].str.split('_').str[
        1]  # right["org_label"].astype(str)
    ...
    if i > 0:  # need to also select organoids that are in the org_df (i.e. linked R0-R1 organoids)
        linked_organoids = (org_df["plate_id"] + "_" + org_df["well_id"] + "_" + org_df["org_label_linked"].astype(
            str)).unique()
    ...
    right_filt["org_label_linked"] = right_filt["organoid_id_linked"].str.split('_').str[1].astype(float).astype(
        'Int64')
...
org_df.to_csv(join(exps['R0'].get_experiment_dir(), ("org_df_linked_" + '-'.join(round_names) + ".csv")),
...
for i in range(len(round_names) - 1):
    right = dfs[round_names[i + 1]]  # load next round's df
    right["id"] = right["plate_id"] + "_" + right["well_id"] + "_" + right["organoid_id"].str.split('_').str[1] + "_" + \
    ...
    if i > 0:  # need to also select nuclei that are already in the nuc_df (i.e. linked R0-R1 nuclei)
        linked_nuclei = (
                    nuc_df["plate_id"] + "_" + nuc_df["well_id"] + "_" + nuc_df["org_label_linked"].astype(int).astype(
                str) + "_" + nuc_df["nuc_id_linked"].astype(int).astype(str)).unique()
    ...
    if i == 0:  # if linking R0-R1, use the R0 df
        left = dfs[round_names[0]]
        left["id"] = left["plate_id"] + "_" + left["well_id"] + "_" + left["organoid_id"].str.split('_').str[1] + "_" + \
    ...
    # WARNING! DID NOT TEST MORE THAN TWO ROUNDS YET, CHECK THAT THIS WORKS
        ...
        # update "id" to match R0 numbering
        left["id"] = left["plate_id"] + "_" + left["well_id"] + "_" + left["organoid_id_linked"].str.split('_').str[
            1] + "_" + left["nuc_id_linked"].astype(int).astype(str)
...
# Save to R0 directory
nuc_df.to_csv(join(exps['R0'].get_experiment_dir(), ("nuc_df_linked_" + '-'.join(round_names) + ".csv")),
              index=False)  # saves csv
