...
def plot_heatmap_means(
    df, feature="C03.mean_intensity", plate_size=96, vmax_multiplier=1
):
    vmin = df[feature].min()
    vmax = df[feature].max()
...
def make_rgb_imagescale(r, g, b, min_quantile=0, max_quantile=0.999):
    ...
    if np.any(r):
        r = rescale_intensity(
            r,
            in_range=tuple(np.quantile(r, [min_quantile, max_quantile])),
            out_range=np.float64,
        )
    if np.any(g):
        g = rescale_intensity(
            g,
            in_range=tuple(np.quantile(g, [min_quantile, max_quantile])),
            out_range=np.float64,
        )
    if np.any(b):
        b = rescale_intensity(
            b,
            in_range=tuple(np.quantile(b, [min_quantile, max_quantile])),
            out_range=np.float64,
        )
...
def plot_image_grid(roi_npimg_dict, brighten=0.5, ncols=None, cmap="gray"):
    ...
    imgs = [
        roi_npimg_dict[cond][obj][0]
        for cond in sorted(set(roi_npimg_dict.keys()))
        for obj in sorted(set(roi_npimg_dict[cond].keys()))
    ]
    plate_id = [
        obj[0]
        for cond in sorted(set(roi_npimg_dict.keys()))
    ...
    umax = np.amax(
        [xmax, ymax]
    )  # set box shape to square, and max of all organoid dims
        ...
        for col in range(ncols):
            ...
            if cond_id[plotted_count] == plot_cond:
                ...
                ax.imshow(
                    img,
                    cmap=cmap,
                    vmax=brighten * np.amax(img),
                    origin="upper",
                    extent=[
                        (umax / 2) - img.shape[1] / 2.0,
                        (umax / 2) + img.shape[1] / 2.0,
                        (umax / 2) - img.shape[0] / 2.0,
                        (umax / 2) + img.shape[0] / 2.0,
                    ],
                )
                ax.set_xlim(0, umax)
...
def plot_rgb_grid(
    ...
    if np.any(r_dict):
        r_imgs = [
            r_dict[cond][obj][0]
            for cond in sorted(set(metadict.keys()))
            for obj in sorted(set(metadict[cond].keys()))
        ]
    if np.any(g_dict):
        g_imgs = [
            g_dict[cond][obj][0]
    ...
    nrows = len(conditions)  # nrows is the number of conditions
    ncols = math.ceil(
        sum(len(v) for v in metadict.values()) / len(metadict)
    )  # ncols is number of samples
        ...
        for col in range(ncols):
            ...
            if cond_id[plotted_count] == plot_cond:
                ...
                ax.imshow(
                    ...
                    origin="upper",
                    extent=[
                        (umax / 2) - rgb_uint8.shape[1] / 2.0,
                        (umax / 2) + rgb_uint8.shape[1] / 2.0,
                        (umax / 2) - rgb_uint8.shape[0] / 2.0,
                        (umax / 2) + rgb_uint8.shape[0] / 2.0,
                    ],
                )
                ax.set_xlim(0, umax)
...
def extract_imgs(img_dict):
    imgs = [
        img_dict[cond][obj][0]
        for cond in sorted(set(img_dict.keys()))
        for obj in sorted(set(img_dict[cond].keys()))
    ]
    return imgs
...
def plot_pos_and_neg_sets(
    ...
    b_neg_dict = load_imgs_from_object_dict(
        neg_objects_randomized,
        zarr_url_dict,
    ...
    g_neg_dict = load_imgs_from_object_dict(
        neg_objects_randomized,
        zarr_url_dict,
    ...
    r_neg_dict = load_imgs_from_object_dict(
        neg_objects_randomized,
        zarr_url_dict,
    ...
    b_pos_dict = load_imgs_from_object_dict(
        pos_objects_randomized,
        zarr_url_dict,
        channel_index=b_ch_idx,
        level=level,
        roi_name=roi_name,
    ...
    g_pos_dict = load_imgs_from_object_dict(
        pos_objects_randomized,
        zarr_url_dict,
        channel_index=g_ch_idx,
        level=level,
    ...
    r_pos_dict = load_imgs_from_object_dict(
        pos_objects_randomized,
        zarr_url_dict,
        channel_index=r_ch_idx,
        level=level,
...
def count_positive_fraction(df, colname, thresh):
    ...
    grouped = (
        df.groupby(["condition", "plate_id", "well_id", "timepoint"])[
            ["count", "pos.count"]
        ]
        .sum()
        .reset_index()
