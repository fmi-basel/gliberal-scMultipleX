...
def annotate_mesh_by_child_features(
    *,
    ...
    # Load child features
    feat_adata = ad.read_zarr(f"{zarr_url}/tables/{child_feature_table}")

    # Initializing saving location
    save_mesh_path = f"{zarr_url}/meshes/{new_mesh_name}"
    os.makedirs(save_mesh_path, exist_ok=True)

    logger.info(f"Saving meshes to directory: /meshes/{new_mesh_name}")
    ...
    for i, obsname in enumerate(adata.obs_names):
        ...
        tree = cKDTree(feat_df[["x_pos_pix", "y_pos_pix", "z_pos_pix_scaled"]].values)
        ...
        for col in annotate_by_features:
            ...
            vtk_intensity.SetName(col)  # Name of the array in VTK
