...
if os.environ.get("save_image_name") is not None:
    save_affine_raw_image_name = (
        os.environ.get("save_image_name")[:-4] + "_affine_raw.tif"
    )
    save_ffd_raw_image_name = (
        os.environ.get("save_image_name")[:-4] + "_affine_ffd_raw.tif"
    )
    save_affine_label_image_name = (
        os.environ.get("save_image_name")[:-4] + "_affine_label.tif"
    )
    save_ffd_label_image_name = (
        os.environ.get("save_image_name")[:-4] + "_affine_ffd_label.tif"
    )
...
# Load point clouds
moving_pc = pd.read_csv(
    moving_pc_file_name, sep=" ", header=None
).to_numpy()  # N x 5 (first column is ids, last column is size )
...
for row in matching_moving_fixed_ids:
    if col_ids[np.where(row_ids == row[1])] == row[0]:
        hits += 1
print("=" * 25)
print(f"Matching Accuracy is {hits / len(matching_moving_fixed_ids):.4f}")
...
np.savetxt(
    os.path.dirname(save_affine_label_image_name) + "/results_affine.csv",
    results_affine,
    fmt="%i %i %1.4f",
    header="moving_id fixed_id confidence",
    comments="",
)
...
print("=" * 25)
print(
    "Matching Accuracy after performing FFD non-rigid registration is {:.4f}".format(
...
np.savetxt(
    os.path.dirname(save_ffd_label_image_name) + "/results_ffd.csv",
    results_ffd,
    fmt="%i %i %1.4f",
    header="moving_id fixed_id confidence",
    comments="",
