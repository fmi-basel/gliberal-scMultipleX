...
def get_centroid(detections, transposed=True):
    ...
    :param transposed: If detections are transposed.
    :return: 3 x 1
    """
    if transposed:  # N x 4
...
def get_error(moving_landmarks, fixed_landmarks):
    ...
    if moving_landmarks is not None or fixed_landmarks is not None:
        ...
        )  # axis= 0 means norm is taken along axis = 0
    else:
        return None
...
def generate_affine_transformed_image(
    transform_matrix, fixed_raw_image, moving_raw_image, moving_label_image
):
    """
    ...
    affine = affine_translate(
        affine, inv_matrix[0, 3], inv_matrix[1, 3], inv_matrix[2, 3]
    )
...
def generate_tps_transformed_image(
    fixed_image,
    moving_image,
    ...
    save_tps_image_name,
    ds_grid_factor=4,
    logger=logging.getLogger("PlatyMatch"),
):
    ...
    for z in tqdm(range(0, moving_image.shape[0])):
        for y in range(0, moving_image.shape[1], ds_grid_factor):
            ...
            transformed_moving_image_coordinates.append(
                apply_tps_transform(
                    np.asarray(moving_image_coordinates),
                    moving_control_points,
                    w_a_x,
                    w_a_y,
                    w_a_z,
                )
            )  # N x 3 (x1 y1 z1; x2 y2 z2; ...)
    transformed_moving_image_coordinates = np.asarray(
        transformed_moving_image_coordinates
    ).reshape(
        (moving_image.shape[0])
        * (moving_image.shape[1] // ds_grid_factor)
        * (moving_image.shape[2] // ds_grid_factor),
        3,
    )
    ...
    logger.info("Applying TPS transform.")
    transformed_tps_image = griddata(
        transformed_moving_image_coordinates, data, fixed_image_coordinates
    )
    transformed_tps_image = transformed_tps_image.reshape(
        fixed_image.shape[0], fixed_image.shape[1], fixed_image.shape[2]
    )
    tifffile.imwrite(
        save_tps_image_name, transformed_tps_image.astype(fixed_image.dtype)
    )

    logger.info(
        "Saving TPS Transformed Image with shape {} at location: {} ".format(
            transformed_tps_image.shape, save_tps_image_name
        )
    )
...
def generate_ffd_transformed_image(
    fixed_image,
    moving_image,
    ...
    """
    Apply FFD transformation to image with BSpline interpolation.
    ...
    Parameters:
        ...
        moving_image: np.array
            Image to transform.
        fixed_image_mask: np.array
            Mask of the fixed image.
        grid_physical_spacing: list[float]
            BSpline control points in physical space.
    """
