### Purpose
- Copy and shift 2D or 3D label image from reference round to moving round(s) using non-rigid transform calculated from 2D label images
- Transform (rotation + translation) label using on the fly 2D (xy) non-rigid transformation of the linked 2D MIP label image
...
- Allows user to propogate segmentation run only on reference round to subsequent rounds, using non-rigid registration
...
- Saves 2D non-rigid transform in 'registration' folder of moving image following NGFF coordinateTransformations specification: JSON file of sequence (rotation, translation) for forward transform from moving to reference image in physical units (y,x).

### Limitations
- Applies x,y translation and rotation only (non-rigid Euclidean transformation only; no scaling or warping)
